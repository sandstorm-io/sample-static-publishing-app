...
if [ -e /etc/nginx/sites-enabled/default ] ; then unlink /etc/nginx/sites-enabled/default ; fi
...
EOF
if [ ! -e /etc/nginx/sites-enabled/sandstorm-php ]; then ln -s /etc/nginx/sites-available/sandstorm-php /etc/nginx/sites-enabled/sandstorm-php ; fi
service nginx stop
